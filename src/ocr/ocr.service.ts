...
export class OcrService {
    ...
    async getBillById(billId: string, userId: string): Promise<any> {
        const bill = await this.billsRepository.findOne({
            where: { id: billId }, relations: ['user'],
        });
        if (!bill) {
    ...
    async initiateBillProcessing(imageBuffer: Buffer, user: User): Promise<Bill> {
        ...
        return billDetails as Bill;
    }

    async processQueuedBill(billId: string): Promise<void> {
        const bill = await this.billsRepository.findOneBy({ id: billId });
        if (!bill || bill.status !== BillStatus.PROCESSING) {
            console.log(`Skipping processing for bill ${billId}. Status: ${bill?.status}`);
            return;
        }

        try {
            const response = await firstValueFrom(this.httpService.get(bill.imageUrl, { responseType: 'arraybuffer' }));
            ...
            bill.items = parsedData.items;
            bill.total = parsedData.total;
            bill.storeName = parsedData.storeName;
            bill.storeLocation = parsedData.location;
            bill.purchaseDate = parsedData.date ? new Date(parsedData.date) : new Date();
            bill.rawText = rawText;
            bill.status = BillStatus.COMPLETED;
        } catch (error) {
            console.error(`Failed to process bill ${billId}:`, error);
            if(bill) bill.status = BillStatus.FAILED;
        } finally {
            if(bill) await this.billsRepository.save(bill);
        }
    }
    ...
    private async extractDataWithGemini(
        ...
        const prompt = `
            ...
            4. "items": Sebuah array objek berisi "name", "quantity", dan "price".
            5. "total": Total akhir dari struk.
            ...
            Hanya kembalikan objek JSON, tanpa penjelasan. Teks struk: --- ${text} ---
        `;
        try {
